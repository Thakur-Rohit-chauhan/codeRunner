...
class SubmissionOrchestrator:
    ...
    async def social_login(self, payload: SocialLoginRequest) -> AuthEnvelope:
        username = normalize_username(payload.username or (payload.email or "google_user").split("@")[0])
        email = (payload.email or f"{username}@coderunner.dev").strip().lower()
        ...
        async with self.session_factory() as session:
            ...
            if not user:
                user = User(
                    ...
                    username=username,
                    email=email,
                    password_hash=hash_password(f"{payload.provider}-oauth"),
                    display_name=payload.displayName or username.replace("_", " ").title(),
                    role="user",
                    rank="Apprentice",
                    rating=1250,
...
def create_app(
    *,
    ...
    @auth_router.post("/social-login", response_model=AuthEnvelope)
    async def social_login(payload: SocialLoginRequest, runtime: SubmissionOrchestrator = Depends(get_runtime)) -> AuthEnvelope:
        return await runtime.social_login(payload)
