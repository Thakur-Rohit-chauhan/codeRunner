...
export default function TopicStats() {
    ...
    // Decode URL param
    const currentTopic = decodeURIComponent(topicName || '')
    ...
    const handlePractice = () => {
        ...
        const firstUnsolved = topicProblems.find(p => p.status !== 'solved')
        if (firstUnsolved) {
            navigate(`/problems/${firstUnsolved.id}?topic=${encodeURIComponent(currentTopic)}`)
        } else if (topicProblems.length > 0) {
            navigate(`/problems/${topicProblems[0].id}?topic=${encodeURIComponent(currentTopic)}`)
        } else {
            navigate('/problems')
        }
    }
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                    ...
                    <div style={{ ...glassCard, padding: '24px' }}>
                        ...
                        <div style={{
                            ...
                            marginBottom: '20px', boxShadow: '0 4px 20px rgba(59, 130, 246, 0.4)'
                        }}>
                            <span style={{ fontSize: '28px', color: '#fff', fontWeight: 800 }}>{currentTopic.charAt(0)}</span>
                        </div>

                        <h1 style={{ fontSize: '24px', fontWeight: 700, margin: '0 0 8px 0', color: '#fff' }}>{currentTopic}</h1>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                        {displayProblems.length === 0 ? (
                            ...
                            displayProblems.map((p, index) => (
                                <div key={p.id} 
                                    onClick={() => navigate(`/problems/${p.id}?topic=${encodeURIComponent(currentTopic)}`)}
                                    style={{
                                    ...glassCard,
                                    padding: '16px 20px',
                                    display: 'grid',
